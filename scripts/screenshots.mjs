 ...
 * and an important row with their accent bars, a one-off wearing its tag, the
 * destination picker and the composer hint. Keep it that way — if a screenshot
 * stops showing a state, that state stops being reviewed.
 *
 * The seed goes through `normalize`, which means it goes through `settle` — so
 * a group's mark has to be spelled out on its items as well as on the group.
 * Written on the group alone it is derived straight back off again, and the
 * screenshot quietly stopped showing the accent bar it is here to show.
 */
...
const DEMO_DAY = {
  ...
  list: [
    ...
    },
    {
      ...
      important: true,
      items: [
        { kind: "task", id: "t3", text: "make calls", target: 3, count: 1, important: true },
        { kind: "task", id: "t4", text: "review the PR", target: 1, count: 1, important: true },
        {
          kind: "task",
          id: "t5",
    ...
    { kind: "task", id: "t7", text: "stretch", target: 1, count: 0, once: true },
  ],
};
